...
#2. temperature gradient needs to be inside the loop? what is wrong with the temperature?
...
lake_biel <- readRDS("lake_test/predictions_Biel.rds")
biel_predictions <- list()
...
df_biel_pred_test <- df_biel |> 
  pivot_longer(
    cols = c(1:22),
    names_to = "Species", 
    values_to = "model_prediction")

df_biel_pred_test <- df_biel_pred |> 
  rename(temp = mean_last_7days)
```

### Efficiently do dfs for pred, deriv and resp diversity
