...
# looking at all NAs in the df, NAs should be gone for the GAM (might be the problem why negbin does not work), does it work when all NAs are gone?
# df_NA <- abu_nets_df_final |> 
#   filter_all(any_vars(is.na(.)))

#301 NAs in Temperature data

sum(is.na(abu_nets_df_final$mean_last_7days))
# 
abu_nets_df_final <- abu_nets_df_final |> 
...
for (i in lakes_list){
  ...
  for (j in species_list){ #looping through all species occuring in this lake
    data <- presence_lake |> 
      filter(Species == j)
    gam_output[[j]] <- gam(data = data, value ~ s(mean_last_7days), family = negbin(1), method = "REML") #GAM per species in all lakes
...
# drop NAs in temp column

# df_small <- df_small |> 
#   drop_na(mean_last_7days)
# 
# sum(is.na(df_small$Species))
# 
...
#     data <- presence_lake |>
#       filter(Species == j)
#     gam_output[[j]] <- gam(data = data, value ~ s(mean_last_7days, k = 4), family = negbin(1), method = "REML") #GAM per species in all lakes
#     model_prediction[[j]] <- predict.gam(gam_output[[j]], temp_gradient, type = "response", se.fit = TRUE)$fit #saving model predictions in RDS
...
# list of problems:
#1. adding negbin not working! works until sarnen, then problems 
...
sum(is.nan(abu_nets_df_final$mean_last_7days))
```
...
```{r}
#what is the problem with negbin? works with small_lakes, in big df always stopping at sarnen third species (Barbus barbus)
# no sarnen test
df_sarnen <- abu_nets_df_final |> 
  filter(Lake == "Sarnen")
...
df_no_sarnen <- abu_nets_df_final |> 
  filter(!Lake == "Sarnen")

nosa_list <- df_no_sarnen |> 
  distinct(Lake) |> 
  pull(Lake)
