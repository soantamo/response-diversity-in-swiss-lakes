...
for (i in lakes_list){
  species_list <- abu_nets_df_final |>
    filter(Lake == i) |> 
    distinct(Species) |>
    pull(Species)
  #saveRDS(species_list,paste0("test_species/species_", i,".rds"))
  for (j in species_list){
    gam_output <- gam(data = abu_nets_df_final |> filter(Species == j), value ~ s(mean_last_7days, k = 3))
    model_prediction[[j]] <- predict.gam(gam_output, temp_gradient, type = "response", se.fit = TRUE)$fit
    saveRDS(model_prediction,paste0("pred_test/predictions_", i , ".rds"))
    derivatives[[j]] <- derivatives(gam_output)
    saveRDS(derivatives, paste0("deriv_test/derivatives_", i, ".rds"))
    
  }
}

species_const<- abu_nets_df_final |>
    filter(Lake == "") |> 
    distinct(Species) |>
    pull(Species)

work <- readRDS("pred_test/predictions_Biel.rds")
works <- readRDS("pred_test/predictions_Zug.rds")
