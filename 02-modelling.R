...
for (i in species_list) {
  ...
  grid <- expand.grid(Depth_sample = seq(
    ...
    fProtocol = unique_method$fProtocol)
  gam_output[[i]] <- gam(data = data, Abundance ~ s(Depth_sample, k = 3) +
                           s(fProtocol, bs = 're'), family = binomial)
  ...
  # Left: a qq-plot to detect overall deviations from the expected distribution
  # Right: a plot of the residuals against the rank-transformed model predictions
  tiff_filename <- paste("model_1/depth/gam_check/gam_check_", i, ".tiff", sep = "")
  tiff(tiff_filename, width = 800, height = 600)
  print(plot(simulationOutput))
  dev.off()
  # get rid of NAs in temp datat
  ...
  # Plotting standardized residuals against predictors
  tiff_file_2 <- paste("model_1/depth/gam_check/predictor_", i, ".tiff", sep = "")
  tiff(tiff_file_2, width = 800, height = 600)
  ...
  pred_df <- model_bind |>
    ...
    mutate(species = factor(i))
  saveRDS(pred_df, paste0("model_1/depth/predictions/predictions_",i,".rds"))
}

# test <- readRDS("model_1/depth/predictions/predictions_Alosa_fallax.rds")
# 
# test |> 
#   ggplot(aes(depth, fit)) +
#   geom_line()

depth_pred_mod1 <- list.files(path = "model_1/depth/predictions", pattern = ".rds", full.names = TRUE) |> 
  map_dfr(readRDS)
...
depth_pred_mod1 |> 
  ggplot(aes(depth, fit, color = factor(species))) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3) +
  theme_bw() +
  facet_wrap(~species, scales = "free") +
