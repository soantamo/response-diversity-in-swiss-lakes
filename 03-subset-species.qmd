...
#only species with significant coefficients and smoothing parameters included, BIel only 
df_small <- abu_nets_df_final |>
  filter(Lake %in% c("Biel", "Brienz", "Walen")) |> 
  filter(Species %in% c("Alburnus_alburnus", "Coregonus_confusus", "Coregonus_sp", "Perca_fluviatilis", "Rutilus_rutilus", "Sander_lucioperca", "Tinca_tinca"))

small_lakes <- df_small_sub|>
  distinct(Lake) |>
  pull(Lake)
